...
## D-004: async 전용 호출과 공통 토큰 버킷

- 상태: accepted
- 날짜: 2026-09-14
- 대체: D-003

사용자 요청에 따라 기존 서비스 속성과 모델을 유지하면서 HTTP, 페이지 순회, debug,
저장의 공개 I/O API를 비동기로 전환한다. 동기 호환 facade는 두지 않는다.
공통 AsyncTokenBucket 구현을 패키지 내부에 독립적으로 포함해 자매 패키지 의존을
추가하지 않고 동일한 TPS·burst·취소 규칙을 적용한다. HTTPX 연결은 첫 요청 때 열어
생성자 실패 시 연결 자원이 남지 않게 한다. 종료는 async with/aclose()로 명시한다.
