...
async def test_live_agri_weather_station_endpoint_returns_items() -> None:
    api_key = _api_key()
    if not api_key:
    ...
    async with DataGoKrClient(api_key=api_key) as client:
        try:
            page = await client.agri_weather.station_list(num_of_rows=2)
...
async def test_live_kwater_sluice_endpoint_returns_items() -> None:
    api_key = _api_key()
    if not api_key:
        pytest.skip("DATA_GO_KR_SERVICE_KEY is required")
    ...
    async with DataGoKrClient(api_key=api_key) as client:
        try:
            page = await client.kwater_sluice.hour_list(
                damcode="2022510",
                stdt="2018-10-01",
                eddt="2018-10-01",
                num_of_rows=2,
